-- Align the database dispatcher's claim key with the Python path.
-- Migration 018 renamed every existing row to 'pre_match_20m', but the dispatcher
-- (created in 006/010) still INSERTs 'pre_match_60m' and its anti-duplicate LEFT JOIN
-- still filters on that stale type. Since 018 no row of that type exists, the ON
-- CONFLICT claim silently stopped guarding against double delivery: the DB dispatcher
-- and the GitHub-run Python path then BOTH send a card for the same fixture.
BEGIN;
...
-- The DB dispatcher claims via pre_match_telegram_queue (PK match_id), while the
-- Python path claims via notification_log. Both claim types now share the
-- 'pre_match_20m' label so each can see the other's claim.
COMMIT;