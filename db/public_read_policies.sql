...
REVOKE ALL ON TABLE leagues, teams, matches, match_stats,
    team_form, player_availability, team_availability_status, fixture_lineups,
    odds_quote_history, predictions
FROM anon, authenticated;

GRANT SELECT ON TABLE leagues, teams, matches, player_availability,
    team_availability_status, fixture_lineups, odds_quote_history, predictions TO anon;
...
ALTER TABLE odds_quote_history ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "public_read_odds_quote_history" ON odds_quote_history;
CREATE POLICY "public_read_odds_quote_history" ON odds_quote_history FOR SELECT TO anon USING (true);
