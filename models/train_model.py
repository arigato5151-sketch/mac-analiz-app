...
def _chronological_slices(size: int) -> tuple[slice, slice, slice]:
    if size < 1_000:
        raise ValueError("At least 1000 chronological matches are required")
    test_start = int(size * 0.80)
    validation_start = int(test_start * 0.85)
...
def train_models(
    ...
    fit_slice, validation_slice, test_slice = _chronological_slices(len(features))
    ...
    btts_probabilities = btts_model.predict_proba(x_test)[:, 1]
