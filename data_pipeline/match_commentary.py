"""Generate bounded Turkish match commentary with the Gemini or NVIDIA NIM API."""
...
def generate_match_commentary(
    *,
    ...
    # Free NVIDIA NIM tier is the automatic fallback once every Gemini model fails;
    # an optional LLM must never block the caller with a configuration error.
    try:
        return _fallback_to_nvidia_commentary(
            home_team=home_team,
            away_team=away_team,
            home_xg=home_xg,
            away_xg=away_xg,
        ...
        raise last_error or MatchCommentaryError(
            "Commentary could not be generated by any provider", reason="provider"
        )


def _fallback_to_nvidia_commentary(
    *,
    home_team: str,
    away_team: str,
    home_xg: float | None,
    ...
    """Try the free NVIDIA NIM tier when Gemini is unavailable; raise MatchCommentaryError."""
    from data_pipeline.nvidia_companion import (
        NvidiaCompanionError,
        generate_nvidia_commentary,
    )
    ...
    try:
        return generate_nvidia_commentary(
            prompt=prompt, api_key=api_key, model_name=model_name
        )
    except NvidiaCompanionError as error:
        raise MatchCommentaryError(
            f"Gemini and NVIDIA NIM commentary unavailable: {error.reason}",
            reason=error.reason,
        ) from error
